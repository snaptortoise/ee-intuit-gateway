...
function event_espresso_txn(){
	...
	if ($attendee_id ==""){
		...
		if (file_exists(EVENT_ESPRESSO_GATEWAY_DIR . "/index.php") || file_exists(EVENT_ESPRESSO_PLUGINFULLPATH."gateways/index.php")){
			...
			}elseif(get_option('events_intuit_active') == 'true'){
				if (file_exists(EVENT_ESPRESSO_GATEWAY_DIR . "/intuit/intuit_ipn.php")){
					//Moved files
					require_once(EVENT_ESPRESSO_GATEWAY_DIR . "/intuit/intuit_ipn.php");
				}elseif (file_exists(EVENT_ESPRESSO_PLUGINFULLPATH."gateways/intuit/intuit_ipn.php")){
					//Default files
					require_once(EVENT_ESPRESSO_PLUGINFULLPATH."gateways/intuit/intuit_ipn.php");
				}
			}else{
				//Send an email if payment gaeways are not found.
				$subject = __('Problem With Your Website Payment IPN','event_espresso');
				$body = __('The IPN for ' . $Organization. ' at ' . get_option('siteurl') . ' is not working properly or has not been setup correctly. Date/time' . date('g:i A'),'event_espresso');
			...
			$body = __('The IPN for ' . $Organization. ' at ' . get_option('siteurl') . ' has not been properly setup and is not working. Date/time' . date('g:i A'),'event_espresso');
			wp_mail($contact, $subject, $body);
		}
		...
		if ($payment_status == 'Completed'){
			...
			wp_redirect(get_option('siteurl').'/?page_id='.$org_options['return_url'] ); 
			//exit;
		}
	}
